...
### GitHub 머지 push가 워크플로를 트리거하지 않은 사건 (2026-07-19)

프로덕션이 3커밋 뒤처진 채 멈췄다. WebKit 레이어 수정도 미배포 상태였다. 더 나쁜 건
**내가 "배포 성공"이라고 3번 연속 오보한 것** — `gh run list --limit 1`이 새 실행이 없으면
*이전 실행*을 돌려주는데 SHA를 대조하지 않고 그대로 믿었다. 같은 run URL이 반복해 나온 게
단서였는데 놓쳤다.

**관측 (머지 8건, 전부 squash·부모 1개)**

| 커밋 | 머지(UTC) | 배포 |
|---|---|---|
| ec8b43d #8 | 09:18 | ✅ |
| 9ce08be #9 / 7fdf691 #10 / a83caeb #11 | 17:32~17:47 | ✅ |
| 47a66f6 #12 / b8d1494 #13 | 18:03~18:05 | ❌ |
| 8106d71 #14 | 18:30 | ❌ |
| 0a4e9a5 (**로컬 git push**) | 18:34 | ✅ |
| 0535b6a #15 | 18:42 | ❌ |

**배제한 가설** — 전부 실측으로 확인했다.

- 머지 방식: 8건 모두 squash(부모 1개)인데 결과가 갈렸다 → 변수 아님.
  이후 PR #16을 **`--merge`(머지 커밋, 부모 2개)로 머지해 직접 확인**했고 역시 트리거되지 않았다 → 확정
- 커밋 메시지의 `[skip ci]` 류 지시어: 실패·성공 커밋 모두 없음
- 워크플로 비활성화: `state=active`, 파일은 최초 커밋 이후 무변경
- 인증: 정상 사용자 토큰(`gho_`, `workflow` 스코프)
- Actions 자체 중단: PR 체크(`pull_request` 이벤트)는 4건 모두 정상 실행됨
- `paths` 필터: 없음. Pages 시간당 빌드 한도: 해당 시간대 배포 3건뿐

**남은 사실**: "GitHub 머지 API가 만든 push"만, 17:50 UTC 이후로 실행이 생성되지 않는다.
로컬 git push는 같은 시간대에도 정상 트리거된다. 여기서부터는 GitHub 내부 상태라 밖에서 못 좁힌다.

**대응**
- `workflow_dispatch` 추가 → `gh workflow run ci --ref main`으로 즉시 복구. 실제로 작동 확인.
  이때 배포 조건을 `event_name == 'push'` → `!= 'pull_request'`로 바꿔야 한다. 안 바꾸면
