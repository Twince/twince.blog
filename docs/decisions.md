...
- **보류→방향 확정(D15)**: 뷰포트 연동 스케일은 D15에서 실측 후 방향을 정했다. ⚠️ 여기 적혀 있던 *"전환 비용은 값 교체 한 줄"*은 **틀린 예측이었다** — `--{page}-scale`은 단위 없는 수라 `clamp()`로 유동화가 원천 불가하고(길이와 수를 섞으면 선언이 드롭된다), 컬럼·헤더·카드 상한이 함께 움직여야 한다. 자세한 건 D15.
...
착수 시 작업 범위: 스케일 계단(페이지별 미디어쿼리) + 1100px 컬럼 8곳 동시 clamp + row 카드 `max-w-[640px]` 상한 연동.
스킵(리스크): tailwind 임포트 단일화(캐스케이드), 오타 rename(TocGenrator/isVisable/sortBylastest — decision과 함께 권장).

⑲ 브레이크포인트 체계 — 값이 페이지마다 흩어져 있고 어디에도 근거가 없다.
사이트 전체 분포: 768(7회) / 1024(5회) / 650(2회) / 800(1회) / 1360(1회).
root·series·postGrid·article은 768을 "모바일"로 쓰는데 about만 650이다(히어로 구조가 달라서로
추정되나 문서화 안 됨). 새 페이지를 추가할 때마다 "이 페이지의 모바일은 몇인가"를 매번 다시 정하게 된다.
- 이미 겪은 비용: about의 640/650이 10px 차이로 공존해 그 구간에서 히어로만 모바일이고 연도 레일은
  세로로 남는 중간 상태가 있었다(2026-07-20에 650으로 통일). 두 컴포넌트를 따로 튜닝하다 서로를 못 본 결과.
- 구조적 문제: 미디어쿼리는 `var()`를 못 써서 브레이크포인트가 **반드시 하드코딩**된다.
  about의 1360은 `--about-col`(1300)에서 유도한 값인데 그 결합을 강제할 방법이 없다 —
  `--about-col`을 바꾸면 조용히 어긋난다. D13의 "같다는 주석만 있고 강제 메커니즘이 없으면 갈라진다"와 같은 형태.
- 후보: (A) `@custom-media`(PostCSS 플러그인) — 숫자를 한 곳에 모아 이름을 붙인다.
  (B) 컨테이너 쿼리 — 섹션 폭 기반 문제(타이틀 거터 등)는 뷰포트가 아니라 컨테이너의 문제라 `--about-col`을 자동 추종한다.
  (C) 애초에 쿼리가 필요 없게 — 예: `max-width: calc(col + 2*inset)` + `padding-inline: inset`.
- ⑱(뷰포트 연동 스케일)과 **같이 결정할 것**. 둘 다 "화면 크기에 따라 무엇을 바꿀 것인가"의 다른 얼굴이고,
  따로 정하면 서로 충돌한다.



## D15. 뷰포트 연동 스케일 — 계단식 스케일 + 컬럼 clamp (②안), 타이포 축 분리는 기각

- **맥락**: 큰 모니터에서 1100px 고정 컬럼이 화면의 43%(2560px)·32%(3440px)만 차지하고, 카드 설명은 전 뷰포트에서 11px로 고정이라 콘텐츠 덩어리 전체가 작게 읽힌다.

### 15-1. 타이포 축 분리는 기각 (데이터가 반박)

- **가설**: `--{page}-scale`이 타이포와 치수를 함께 곱하니, 타이포 전용 노브(`--{page}-type-scale`)를 빼면 모바일에서 절대값으로 탈출하는 일이 없어진다.
- **반증**: 모바일/데스크탑 폰트 비율을 4개 페이지에서 실측하니 **0.73~1.18로 흩어졌고 방향까지 반대**였다.

  | 요소 | 비율 | | 요소 | 비율 |
  |---|---|---|---|---|
  | series 페이지 타이틀 | 0.73 | | about EXPERIENCE | 1.00 |
  | series 아이템 타이틀 | 0.74 | | posts 카드 설명 | 1.08 |
  | root 섹션 타이틀 | 0.81 | | root 카드 타이틀 | 1.03 |
  | posts 카드 타이틀 | 0.94 | | root 카드 설명 | 1.18 |

  제목은 공간 제약으로 **작아지고**, 카드 본문은 가독성 때문에 **커진다**. 하나의 스케일로 묶을 수 없다.
  (초기에 "≈0.74로 균일"이라 판단했던 건 series 제목 3개만 보고 일반화한 오류였다.)
- **결정**: **축을 분리하지 않는다.** 서로 다른 이유로 정해진 값을 한 노브에 묶으면 거짓 결합이 된다.
- **모바일 하드코딩은 부채가 아니다**: 모바일은 데스크탑의 축소판이 아니라 다른 레이아웃이다(치수 비율 0.84/0.36/0으로 제각각). 축소판이 아닌 것을 스케일로 표현하면 오히려 왜곡된다. 확장 대비는 축 분리가 아니라 **D13의 타이포 계약 3조**(하한은 불변식 / 공유 역할은 semantic / 페이지 전용은 올리지 말 것)로 이미 충분하다.

### 15-2. ⑱은 ②안(계단식 스케일 + 컬럼 clamp)

- **기술 제약(실측)**: `--card-scale`은 **단위 없는 수**라 `clamp(1.1, …vw, 1.5)`가 무효다(길이와 수 혼합 → 선언 자체가 드롭되어 요소가 전체 폭으로 퍼진다). 유동화하려면 `--card-unit: clamp(1px, …vw, 1.36px)` + 모든 raw를 무단위로 바꿔야 하는데 호출 지점이 213곳이라 과하다. 컬럼 `max-width`는 길이라 clamp 가능.
- **후보 비교** (root, 카드 폭/텍스트 폭/폰트 실측):

  | 후보 | 2560px에서 | 판정 |
  |---|---|---|
  | ① 계단식 스케일만(컬럼 고정) | 썸네일 207→273으로 커지는데 컬럼이 고정이라 **텍스트 폭 278→204(-27%)** | 탈락 |
  | ③ 컬럼만 유동 | 카드 640까지 넓어지나 **폰트 11px 그대로** — 원래 불만이 안 풀림 | 탈락 |
  | **② 계단식 + 컬럼 clamp** | 점유 43→**61%**, 카드 485→640, 텍스트 278→**367**, 폰트 11→**14.5** | **채택** |

- **②의 부수 조건(실측 확인)**:
  1. 헤더가 `max-w-[1100px]` 고정이라 컬럼만 넓히면 세로 기준선이 어긋난다(1920에서 −45px, 2560에서 **−230px**). 같은 1100px이 **8곳**(header/root hero/root recent/posts/series/RelatedPosts/about)에 있어 동시 수정 필요.
  2. row 카드 `max-w-[640px]` 상한이 2560px에서 정확히 걸린다. 그 위로는 컬럼만 넓어지고 카드는 멈춰 다시 작아 보이므로 상한도 스케일 연동 필요.
- **계단 지점**: 1600/2100 2단이 기본. 점프가 거슬리면 1500/1700/1900/2100/2400으로 잘게 쪼개면 거의 연속처럼 보이고 비용은 같다(미디어쿼리만 증가).
- **미확인**: article 본문 컬럼(별도 레이아웃, 이미 clamp 타이포라 영향 덜함)과 `--about-col: 1300px`(혼자 다른 축)은 이번 측정 범위 밖 — 착수 시 함께 판단.
- **전이**: 디자인 토큰을 "단위 없는 배율"로 설계하면 뷰포트 연동으로 갈 길이 막힌다(CSS에서 길이→수 변환이 없다). 유동화 가능성을 남기려면 배율이 아니라 **단위(length)를 노브로** 두는 편이 낫다. 또한 비례 확대는 *한 축만* 키우면 반드시 다른 축을 짜부라뜨린다 — 스케일을 키우려면 그것을 담는 그릇(컬럼)도 같이 키워야 한다.

---

## D16. 검색 노출 — 크롤 기반은 깔고, 페이지별 메타는 ⑧으로 남긴다

2026-07-20. OG 태그를 붙이면서 검색 노출 전반을 점검했다.

**깐 것**: sitemap(`@astrojs/sitemap`) + robots.txt.

robots.txt는 `public/` 정적 파일이 아니라 `src/pages/robots.txt.ts` 엔드포인트다.
sitemap URL을 `astro.config`의 `site`에서 파생시키기 위해서 — 도메인을 두 곳에 적으면 드리프트한다.
파비콘 생성기와 같은 원칙(단일 출처 + 생성물).

**robots.txt를 한 그룹으로 유지하는 이유**: 크롤러는 자기에게 맞는 그룹 **하나만** 따르고
그룹을 병합하지 않는다. `User-agent: GPTBot` 블록을 추가하는 순간 GPTBot은 `*` 규칙을 통째로 무시한다.
봇별 정책이 필요해지면 공통 규칙을 각 그룹에 **복제**해야 한다 — 이걸 모르면 조용히 전면 허용이 된다.

**AI 크롤러 전면 허용(학습·검색 모두)**. 답변 엔진(ChatGPT·Perplexity·AI 개요)에 인용되려면
차단이 불가능하다. 학습만 막고 검색은 허용하는 절충은 봇 구분이 업체 정책에 의존해 불안정하다.
※ `Google-Extended`는 Gemini·Vertex 학습만 통제하고 **구글 AI 개요는 통제하지 못한다**(Googlebot 소관).

**Disallow는 색인 제거가 아니다**: 크롤을 막을 뿐이라, 외부 링크가 있으면 URL만 색인된다.
색인에서 빼려면 `noindex` 메타가 필요한데 그건 크롤러가 페이지를 열어야 보인다 —
`Disallow` + `noindex`는 서로를 무력화한다. 나중에 draft를 숨길 때 반드시 밟을 함정.

**남긴 것 — ⑧이 병목이다.** 페이지별 title·description이 없으면 나머지 SEO 작업은 효과가 안 난다
(검색 결과의 클릭을 결정하는 유일한 요소인데 전 페이지가 "TWINCE.BLOG"다).
데이터는 이미 있다 — `seo.title`/`seo.description`은 스키마에 있고 `resolver.ts`가 `title`/`description`
폴백까지 채운다. 없는 건 `<head>`까지 나르는 props 통로뿐이고, 그건 컴포넌트 계약이라 사용자 EDGE다.

⑧ 이후 순서: JSON-LD(`BlogPosting`) → `og:type=article` + 발행일 → 네이버 서치어드바이저 등록.
**한국어 블로그면 네이버가 구글보다 유입이 클 수 있다**(크롤러 `Yeti`, 별도 사이트 등록 필요).

**AEO(답변 엔진 인용)는 마크업이 아니라 글쓰기다.** 답변 엔진은 페이지가 아니라 **청크(문단) 단위로**
검색해서 문단 하나를 뜯어 근거로 쓴다. 따라서 각 H2/H3 바로 아래 문단이 그 제목만으로 자립해야 하고,
"앞서 말한 것처럼"·"이것은" 같은 상호 참조와 대명사는 청킹에서 지시 대상을 잃는다. 결론을 먼저 쓴다.
다만 AEO는 아직 어리고 인과 측정이 사실상 불가능하다 — 확실한 건 구조화 데이터와 문서 구조뿐이고,
`llms.txt`는 주요 업체 중 사용을 확인한 곳이 없어 채택하지 않았다.
