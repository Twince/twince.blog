...
6. **주석을 쓰기 전에 없앨 방법을 먼저 찾는다.** 값에 이름 붙이기 → 구조로 강제 →
   커밋 메시지·`docs/`로 이관 순으로 시도한다. 코드 밖 사실에서 온 매직 넘버만 한 줄로 남긴다.
   설명문을 코드에 넣지 않는다. (예: `z-index: 2` + 6줄 설명 → `--about-layer-photo: 2`)
7. **페인트 순서를 암묵적 순서에 기대지 않는다.** 무엇이 위에 그려질지를 DOM 순서·flex `order`에
   맡기면 엔진마다 갈린다 — WebKit은 `transform` 요소를 합성 레이어로 올려 z-index를 무시한다.
   레이어 순서는 이름 붙인 z-index로 명시한다(`--about-layer-*`).
   ※ 캐스케이드 소스 순서는 여기 해당 없다. 명세에 정의돼 있고 엔진 간 차이가 없어서,
     미디어쿼리로 base 규칙을 덮는 것은 정상적인 CSS다.
